...
- [x] T7 E2E 검증: `tests/e2e_smoke.sh` — Stop 훅 replay로 감지→대기→주입→정리 전
  ...
  라이브 확인 필요.
- [x] T8 폴백/상태: (a) `pane report-metadata` 상태바("⏳ 재시도 HH:MM"), (b) `watch`
  데몬 — `agent list`로 claude pane 열거 + 세션 transcript 직접 폴링(스크린 스크레이프
  대신 견고), 훅 미발화 폴백 + 고아 마커 waiter 복구. 중복 스케줄 방지 가드 추가.
  (herdr blocked 상태 트리거는 T1에서 불가로 확인돼 제외.)
